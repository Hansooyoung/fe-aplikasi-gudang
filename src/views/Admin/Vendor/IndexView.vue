...
const fetchDataVendor = async () => {
  try {
    ...
    vendorData.value = response.data.data;
  } catch (error) {
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/vendor/${itemToDelete.value.id}`,
      headers: {
        'Accept': 'application/json',
        'Content-type': 'application/json',
      },
...
const confirmDelete = (vendor) => {
  itemToDelete.value = vendor;
  modalMessage.value = 'Apakah Anda yakin ingin menghapus vendor ini?';
...
<template>
  <DefaultLayoutAdmin>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Vendor</h4>
      <div class="max-w-full overflow-x-auto">
        <div class="flex justify-start mb-4">
          <button 
            @click="$router.push({ name: 'vendor.create' })"
          ...
          <tbody>
            <tr v-for="(vendor, index) in vendorData" :key="index">
              <td class="py-5 px-4 pl-9 xl:pl-11">
                <h5 class="font-medium text-black dark:text-white">{{ vendor.id }}</h5>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ vendor.nama_vendor }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  <button @click="$router.push({ name: 'vendor.update', params: { id: vendor.id } })" class="hover:text-primary ">Edit</button>
                  <button @click="confirmDelete(vendor)" class="hover:text-primary">Delete</button>
                </div>
              </td>
            </tr>
          </tbody>
