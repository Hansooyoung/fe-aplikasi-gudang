...
const exportToPDF = async () => {
  try {
    ...
    link.download = 'laporan-jenis-barang.pdf'
    link.click()
...
const exportToExcel = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = 'laporan-jenis-barang.xlsx'
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
    ...
    >
      ...
      <div class="max-w-full overflow-x-auto">
        <div class="flex justify-start mb-4">
          <button
            @click="$router.push({ name: 'jenis-barang.create' })"
            ...
            <tr v-for="(jenisBarang, index) in jenisBarangData" :key="index">
              ...
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  <button
                    @click="
                      $router.push({
                        ...
                        params: { jenis_barang_kode: jenisBarang.jenis_barang_kode }
                      })
                    "
                  ...
                  <button @click="confirmDelete(jenisBarang)" class="hover:text-primary">
                    Delete
                  </button>
                </div>
              </td>
     ...
     <div
      ...
      >
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
          <h3 class="text-xl font-semibold mb-4">Import Jenis Barang</h3>
