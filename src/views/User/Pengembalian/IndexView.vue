...
// Fetch data pengembalian berdasarkan status
const fetchDataPengembalian = async () => {
  try {
    const url = selectedStatus.value
      ? `/pengembalian?status_kembali=${selectedStatus.value}`
      : '/pengembalian';
...
watch(selectedStatus, fetchDataPengembalian);
...
<template>
  <DefaultLayoutUser>
    ...
    >
      ...
      <select v-model="selectedStatus" class="mb-4 p-2 border rounded-lg">
      ...
      <div class="max-w-full overflow-x-auto">
        ...
        <table class="w-full table-auto">
          ...
          <tbody>
            <tr v-for="(pengembalian, index) in pengembalianData" :key="index">
