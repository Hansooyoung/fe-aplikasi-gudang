...
const fetchDataSiswa = async () => {
  try {
    ...
    // Set response data ke state siswaData
...
const deleteItem = async () => {
  ...
  try {
    await api.request({
      method: 'DELETE',
      url: `/siswa/${itemToDelete.value.id}`,
      headers: {
        'Accept': 'application/json',
        'Content-type': 'application/json',
      },
...
<template>
  <DefaultLayoutUser>
    ...
    >
      <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Siswa</h4>
      <div class="max-w-full overflow-x-auto">
        <div class="flex justify-start mb-4">
          <button 
            @click="$router.push({ name: 'siswa.create' })"
          ...
          <tbody>
            <tr v-for="(siswa, index) in siswaData" :key="index">
              <td class="py-5 px-4 pl-9 xl:pl-11">
                <h5 class="font-medium text-black dark:text-white">{{ siswa.nisn }}</h5>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ siswa.nama }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ siswa.kelas.nama_kelas }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ siswa.jurusan.nama_jurusan }}</p>
              </td>
              <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ siswa.no_hp }}</p>
              </td>
              <td class="py-5 px-4">
                <div class="flex items-center space-x-3.5">
                  <button
                    @click="$router.push({ name: 'siswa.update', params: { id: siswa.id } })"
                  ...
                  <button @click="() => { itemToDelete = siswa; showModal = true; }"
                   class="hover:text-primary">
                    <svg
                      class="fill-current"
                      width="18"
